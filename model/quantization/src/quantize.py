...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(boxes_array) == 0:
            return self.__getitem__((idx + 1) % len(self.img_names))  # Skip this image if no valid boxes

        boxes_tensor = torch.as_tensor(boxes_array, dtype=torch.float32)

        if boxes_tensor.ndim == 2 and boxes_tensor.shape[1] >= 4:
            area_tensor = (boxes_tensor[:, 3] - boxes_tensor[:, 1]) * (boxes_tensor[:, 2] - boxes_tensor[:, 0])
        else:
            raise ValueError(f"Expected boxes_tensor to be a 2D tensor with at least 4 columns, but got shape {boxes_tensor.shape}")
        ...
        if self.transforms:
            sample = self.transforms(image=image, bboxes=target["boxes"], labels=labels_tensor)
            image = sample['image']
...
def postprocessing(x):
    ...
    for i in range(3):
        bs, na_3, ny, nx = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85)
        na = na_3//3
        x[i] = x[i].view(bs, 3, na, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
...
def quantize(build_dir, quant_mode, weights, dataset, data):
    ...
    # quantizer.quant_model = quantized_model
    with torch.no_grad():
        for image, target in test_loader:
            ...
            print("predi shape:", output[0].shape)
            pred = postprocessing(output)
            print("pred shape:", pred[0].shape)
            pred_final = non_max_suppression(pred)
            print(pred_final)
            # break
...
def run_main():
  ...
  ap.add_argument('-dd',  '--data',  type=str,  default='./data/blind_guide.yaml', help='Path to your data.yaml definition' )
  args = ap.parse_args()
  ...
  quantize(args.build_dir, args.quant_mode, args.weights, args.dataset, args.data)
  return
